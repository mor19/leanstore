...
void TPCCWorkload<AdapterType>::Delivery(Integer w_id, Integer carrier_id, Timestamp datetime) {
  for (Integer d_id = 1; d_id <= D_PER_WH; d_id++) {
    ...
    if (o_id == MinInteger) {  // Should rarely happen
      spdlog::debug("WARNING(1): delivery tx skipped for warehouse(%d), district(%d)", w_id, d_id);
      continue;
    }
    ...
    if (manually_handle_isolation_anomalies) {
      ...
      if (ol_cnt == MinInteger) {
        spdlog::debug("WARNING(2): delivery tx skipped for warehouse(%d), district(%d)", w_id, d_id);
        continue;
      }
    } else {
    ...
    if (manually_handle_isolation_anomalies) {
      ...
      if (!is_safe_to_continue) {
        spdlog::debug("WARNING(3): Isolation chk failed for warehouse(%d), district(%d)", w_id, d_id);
        continue;
      }
    }
    ...
    if (manually_handle_isolation_anomalies) {
      ...
      if (!is_safe_to_continue) {
        spdlog::debug("WARNING(4): Isolation chk failed for warehouse(%d), district(%d)", w_id, d_id);
        continue;
      }
    }
...
void TPCCWorkload<AdapterType>::LoadItem() {
  ...
  spdlog::debug("Load items successfully");
}
...
void TPCCWorkload<AdapterType>::LoadWarehouse() {
  ...
  spdlog::debug("Load warehouses successfully");
}
