#include "benchmark/tpch/workload.h"
...
//                                          const std::vector<Integer> &line_numbers, const std::vector<Integer>
//                                          &supwares, const std::vector<Integer> &item_ids, const std::vector<Integer>
//                                          &quantity_s, Timestamp timestamp) {
...
//   // Update District
//   DELTA_GENERATOR2(district_delta, DistrictType, d_tax, d_next_o_id);
...
//     DELTA_GENERATOR4(stock_delta, StockType, s_quantity, s_remote_cnt, s_order_cnt, s_ytd);
//     stock.UpdateInPlace(
//       {supwares[idx], item_ids[idx]},
//       [&](StockType &rec) {
...
//     Numeric i_price = item.LookupField({itemid}, &ItemType::i_price);  // TODO(XXX): rollback on miss
...
//     DELTA_GENERATOR1(order_delta, OrderType, o_carrier_id);
//     order.UpdateInPlace({w_id, d_id, o_id}, [&](OrderType &rec) { rec.o_carrier_id = carrier_id; }, order_delta);
...
//       DELTA_GENERATOR1(orderline_delta, OrderLineType, ol_delivery_d);
...
//     DELTA_GENERATOR2(customer_delta, CustomerType, c_balance, c_delivery_cnt);
...
//       if (key.o_w_id == w_id && key.o_d_id == d_id && key.o_c_id == c_id) { o_id = key.o_id; }
//       return false;
//     });
//   } else {
...
//   DELTA_GENERATOR1(warehouse_delta, WarehouseType, w_ytd);
//   warehouse.UpdateInPlace({w_id}, [&](WarehouseType &rec) { rec.w_ytd += h_amount; }, warehouse_delta);
//   DELTA_GENERATOR1(district_delta, DistrictType, d_ytd);
...
//     DELTA_GENERATOR4(customer_delta, CustomerType, c_data, c_balance, c_ytd_payment, c_payment_cnt);
...
template <template <typename> class AdapterType>
void TPCHWorkload<AdapterType>::LoadPart() {
  Integer end = PART_SCALE * FLAGS_tpch_scale_factor;
  for (Integer i = 1; i <= end; i++) {
    part.Insert({i}, {RandomString<55>(0, 55), RandomString<25>(25, 25), RandomString<10>(10, 10),
                      RandomString<25>(0, 25), UniformRand(1, 50) * 5, RandomString<10>(10, 10),
                      RandomNumeric(0.0000, 100.0000), RandomString<23>(0, 23)});
  }
  last_part_id = end;
}

template <template <typename> class AdapterType>
void TPCHWorkload<AdapterType>::LoadSupplier() {
  Integer end = SUPPLIER_SCALE * FLAGS_tpch_scale_factor;
  for (Integer i = 1; i <= end; i++) {
    supplier.Insert({i}, {RandomString<25>(25, 25), RandomString<40>(0, 40), this->getNationID(),
                          RandomString<15>(15, 15), RandomNumeric(0.0000, 100.0000), RandomString<101>(0, 101)});
  }
  last_supplier_id = end;
}

template <template <typename> class AdapterType>
void TPCHWorkload<AdapterType>::LoadPartSuppLineItem() {
  Integer part_end  = PART_SCALE * FLAGS_tpch_scale_factor;
  Integer order_end = ORDERS_SCALE * FLAGS_tpch_scale_factor;
  // generate order keys from 1 to order_end
  if (order_end > 1) std::cout << "Generating and shuffling order keys..." << std::endl;
  std::vector<Integer> order_keys(order_end);
  std::iota(order_keys.begin(), order_keys.end(), 1);
  std::random_shuffle(order_keys.begin(), order_keys.end());

  // Load partsupp and lineitem
  const Integer partsupp_size = (PARTSUPP_SCALE / PART_SCALE) * (part_end);
  const Integer lineitem_size = (LINEITEM_SCALE / ORDERS_SCALE) * (order_end);
  auto current_order_key      = order_keys.begin();
  auto lineitem_cnt_in_order  = UniformRand(1, LINEITEM_SCALE / ORDERS_SCALE * 2 - 1);
  int lineitem_number         = 1;
  // (part, supplier) and (order, lineitem) pairs are not perfectly independent,
  // but (part, supplier) and (order) are.
  for (Integer i = 1; i <= part_end; i++) {
    printProgress("parts of partsupp and lineitem", i, 1, part_end);
    // Randomly select suppliers for this part
    size_t supplier_cnt         = UniformRand(1, PARTSUPP_SCALE / PART_SCALE * 2 - 1);
    std::set<Integer> suppliers = {};
    while (suppliers.size() < supplier_cnt) {
      Integer supplier_id = UniformRand(1, last_supplier_id);
      suppliers.insert(supplier_id);
    }
    for (auto &s : suppliers) {
      // load 1 partsupp
      partsupp.Insert({i, s}, {UniformRand(1, 100000), RandomNumeric(0.0000, 100.0000), RandomString<199>(0, 199)});
      // load lineitems
      Integer lineitem_cnt_ps = UniformRand(0, lineitem_size / partsupp_size * 2);
      ...
      for (Integer l = 0; l < lineitem_cnt_ps; l++) {
        lineitem.Insert({*current_order_key, lineitem_number},
                        LineItemType{this->getPartID(), this->getSupplierID(), RandomNumeric(0.0000, 100.0000),
                                     RandomNumeric(0.0000, 100.0000), RandomNumeric(0.0000, 100.0000),
                                     RandomNumeric(0.0000, 100.0000), RandomString<1>(1, 1), RandomString<1>(1, 1),
                                     RandomString<10>(10, 10), RandomString<10>(10, 10), RandomString<10>(10, 10),
                                     RandomString<25>(25, 25), RandomString<10>(10, 10), RandomString<44>(44, 44)});
        lineitem_number++;
        if (lineitem_number > lineitem_cnt_in_order) {
          lineitem_number = 1;
          current_order_key++;
          lineitem_cnt_in_order = UniformRand(1, LINEITEM_SCALE / ORDERS_SCALE * 2 - 1);
          if (current_order_key == order_keys.end()) { current_order_key = order_keys.begin(); }
        }
      }
    }
  }
  ...
  if (rem_order_cnt > 0) std::cout << rem_order_cnt << " orders left to fill out lineitems" << std::endl;
  ...
  for (; current_order_key < order_keys.end(); current_order_key++) {
    ...
    Integer lineitem_cnt = UniformRand(1, LINEITEM_SCALE / ORDERS_SCALE * 2 - 1);
    for (Integer j = 1; j <= lineitem_cnt; j++) {
      // look up partsupp
      auto p         = UniformRand(1, last_part_id);
      auto s         = UniformRand(1, last_supplier_id);
      auto start_key = PartSuppType::Key{p, s};
      bool found     = false;
      partsupp.Scan(
        start_key,
        [&](const PartSuppType::Key &k, const PartSuppType &) {
          p = k.ps_partkey;
          s = k.ps_suppkey;
      ...
      assert(found);
      // std::cout << p << " " << s << " ";
      lineitem.Insert({*current_order_key, j},
                      LineItemType{this->getPartID(), this->getSupplierID(), RandomNumeric(0.0000, 100.0000),
                                   RandomNumeric(0.0000, 100.0000), RandomNumeric(0.0000, 100.0000),
                                   RandomNumeric(0.0000, 100.0000), RandomString<1>(1, 1), RandomString<1>(1, 1),
                                   RandomString<10>(10, 10), RandomString<10>(10, 10), RandomString<10>(10, 10),
...
template <template <typename> class AdapterType>
void TPCHWorkload<AdapterType>::LoadCustomer() {
  Integer end = CUSTOMER_SCALE * FLAGS_tpch_scale_factor;
  for (Integer i = 1; i <= end; i++) {
    customer.Insert({i},
                    {RandomString<25>(0, 25), RandomString<40>(0, 40), this->getNationID(), RandomString<15>(15, 15),
                     RandomNumeric(0.0000, 100.0000), RandomString<10>(10, 10), RandomString<117>(0, 117)});
  }
  last_customer_id = end;
}

template <template <typename> class AdapterType>
void TPCHWorkload<AdapterType>::LoadOrders() {
  Integer end = ORDERS_SCALE * FLAGS_tpch_scale_factor;
  for (Integer i = 1; i <= end; i++) {
    orders.Insert({i},
                  {this->getCustomerID(), RandomString<1>(1, 1), RandomNumeric(0.0000, 100.0000), RandomString<10>(10, 10),
                   RandomString<15>(15, 15), RandomString<15>(15, 15), UniformRand(0, 5), RandomString<79>(0, 79)});
  }
  last_order_id = end;
}

template <template <typename> class AdapterType>
void TPCHWorkload<AdapterType>::LoadNation() {
  for (Integer i = 1; i <= NATION_COUNT; i++) {
    nation.Insert({i}, {RandomString<25>(1, 25), this->getRegionID(), RandomString<152>(0, 152)});
  }
}

template <template <typename> class AdapterType>
void TPCHWorkload<AdapterType>::LoadRegion() {
  for (Integer i = 1; i <= REGION_COUNT; i++) {
    region.Insert({i}, {RandomString<25>(1, 25), RandomString<152>(0, 152)});
  }
}

// -------------------------------------------------------------------------------------
// template <template <typename> class AdapterType>
// auto TPCHWorkload<AdapterType>::NextTransactionArrivalTime(const std::function<void()> &idle_fn) -> uint64_t {
...
template struct TPCHWorkload<LeanStoreAdapter>;
