...
struct BTreeNodeHeader {
  static constexpr u64 SIZE_UNDER_FULL = 3 * PAGE_SIZE / 4;  // merge nodes below this size
  ...
  bool is_leaf;
  leng_t count       = 0;
  leng_t space_used  = 0;                               // space used to store all key + payload
  ...
  auto operator==(const BTreeNodeHeader &rhs) const -> bool;
};

/**
 * @brief Similar to BTreeNodeHeader, but have an embedded HybridLatch for synchronization
 *
 * Main purpose is to implement an in-memory B-tree
 */
struct BTreeNodeHeaderWithLatch : BTreeNodeHeader {
...
class alignas(PAGE_SIZE) BTreeNodeImpl : public PageHeader {
 public:
  ...
  explicit BTreeNodeImpl(bool is_leaf);
  BTreeNodeImpl(BTreeNodeImpl<NodeHeader> &other);
  ~BTreeNodeImpl() = default;
  ...
  void CopyKeyValue(BTreeNodeImpl *dst, leng_t src_slot, leng_t dst_slot);
  void CopyKeyValueRange(BTreeNodeImpl *dst, leng_t dst_slot_start, leng_t src_slot_start, leng_t src_count);

  // Split/Merge utilities
  auto FindSeparator(const ComparisonLambda &cmp) -> SeparatorInfo;
  void InsertSeparator(std::span<u8> separator, pageid_t left_pid, pageid_t right_pid, const ComparisonLambda &cmp);
