...
auto BTreeNodeHeader::operator==(const BTreeNodeHeader &rhs) const -> bool {
  return std::memcmp(this, &rhs, sizeof(BTreeNodeHeader)) == 0;
}
...
template <class NodeHeader>
BTreeNodeImpl<NodeHeader>::BTreeNodeImpl(BTreeNodeImpl<NodeHeader> &other) : header(other.header.is_leaf) {
  std::memcpy(reinterpret_cast<u8 *>(this), reinterpret_cast<u8 *>(&other), sizeof(PageHeader));
}
...
// -------------------------------------------------------------------------------------
template <class NodeHeader>
auto BTreeNodeImpl<NodeHeader>::FindSeparator(const ComparisonLambda &cmp) -> SeparatorInfo {
  ...
  if (header.count > 16 && cmp.op == ComparisonOperator::MEMCMP) {
    // Prefix optimization is only for MEMCMP
...
template <class NodeHeader>
void BTreeNodeImpl<NodeHeader>::InsertSeparator(std::span<u8> separator, pageid_t left_pid, pageid_t right_pid,
                                                const ComparisonLambda &cmp) {
  assert(IsInner());
  leng_t old_slot = LowerBound(separator, cmp);
  if (old_slot == header.count) {
    assert(header.right_most_child == left_pid);
    header.right_most_child = right_pid;
  } else {
    assert(GetChild(old_slot) == left_pid);
    std::memcpy(GetPayload(old_slot).data(), &right_pid, sizeof(pageid_t));
  }
  InsertKeyValue(separator, {reinterpret_cast<const u8 *>(&left_pid), sizeof(pageid_t)}, cmp);
}
...
void BTreeNodeImpl<NodeHeader>::SplitNode(BTreeNodeImpl<NodeHeader> *parent, BTreeNodeImpl<NodeHeader> *node_right,
  ...
  BTreeNodeImpl<NodeHeader> tmp(*this);
  BTreeNodeImpl<NodeHeader> *node_left = &tmp;

  // update_fence of two child nodes
  node_left->SetFences(GetLowerFence(), sep_key, cmp);
  if (node_right != nullptr) { node_right->SetFences(sep_key, GetUpperFence(), cmp); }

  // update the separator in parent node
  if (parent != nullptr) { parent->InsertSeparator(sep_key, left_pid, right_pid, cmp); }
  ...
  node_left->MakeHint();
  if (node_right != nullptr) { node_right->MakeHint(); }
...
auto BTreeNodeImpl<NodeHeader>::MergeNodes(leng_t left_slot_id, BTreeNodeImpl<NodeHeader> *parent,
  ...
  BTreeNodeImpl<NodeHeader> tmp(*right);
  ...
  if (header.is_leaf) {
    ...
    if (space_upper_bound > PAGE_SIZE) { return false; }
    /**
     * @brief Moving keys to the new node, including:
     * - (1) All keys from the current node (which is the 'left' node)
     * - (2) The key in the parent node (which splits between left and right nodes)
     *   This should be coupled with left->right_most_child
     * - (3) All keys from the right node
     */
    // (1) start moving entries to new node
    CopyKeyValueRange(&tmp, 0, 0, header.count);
    // (2) extra key are retrieved from {parent->full_key[left_slot_id], left->right_most_child}
