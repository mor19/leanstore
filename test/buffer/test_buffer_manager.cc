...
TEST_F(TestBufferManager, BasicTest) {
  ...
  spdlog::info("Start evict all pages");
  ...
  spdlog::info("Read back evicted pages");
...
TEST_F(TestBufferManager, BasicTestWithExtent) {
  ...
  EXPECT_THAT(start_pids[0], ::testing::AllOf(::testing::Ge(N_PAGES + 1), ::testing::Le(N_PAGES * 2 + 1)));
  EXPECT_EQ(buffer_->physical_used_cnt_, total_ext_size + 1);
  for (auto idx = 0U; idx < extent_count; idx++) {
    EXPECT_THAT(start_pids[idx], ::testing::AllOf(::testing::Ge(N_PAGES + 1), ::testing::Le(N_PAGES * 2 + 1)));
  ...
  spdlog::info("Modify these extents");
  for (auto idx = 0U; idx < extent_count; idx++) {
    ...
    // Check frame info
    EXPECT_EQ(buffer_->ext_frame_[start_pids[idx]].prevent_evict.load(), true);
    EXPECT_EQ(buffer_->GetPageState(start_pids[idx]).Version(), 1);
  }

  // Flush & then Evict all these extents
  spdlog::info("Mark this BLOB as EVICTED");
  auto ret = 0;
  for (auto idx = 0U; idx < extent_count; idx++) {
  ...
  spdlog::info("Evict the whole buffer pool");
  ...
  spdlog::info("Read back evicted extents using large page API");
