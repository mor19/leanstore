...
class TestExtendedBTree : public BaseTest {
 protected:
  std::unique_ptr<ExtendedBTree> tree_;
...
TEST_F(TestExtendedBTree, InsertAndQuery) {
...
TEST_F(TestExtendedBTree, RemoveAndQuery) {
  std::vector<std::pair<int, __uint128_t>> data;
...
TEST_F(TestExtendedBTree, UpdateAndQuery) {
  ...
  for (auto idx = 0ULL; idx < NO_RECORDS; idx++) {
    ...
    auto success        = tree_->Update(key, payload, [&](std::span<u8> prev) {
      int prev_num;
...
TEST_F(TestExtendedBTree, TreeScan) {
  std::vector<std::pair<int, int>> data;
  PrepareData<int>(data, false);
...
TEST_F(TestExtendedBTree, ConcurrentInsertAndSearch) {
  std::vector<std::pair<int, int>> data;
