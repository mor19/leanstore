...
class TestBTreeNodeWithTimeStamp : public ::testing::Test {
 protected:
  storage::Page mem_;
...
TEST_F(TestBTreeNodeWithTimeStamp, Constructor) {
  auto node = new (&mem_) storage::BTreeNodeWithTimeStamp(true);
...
TEST_F(TestBTreeNodeWithTimeStamp, NormalOperation) {
  auto node = new (&mem_) storage::BTreeNodeWithTimeStamp(true);
  ...
  for (auto &pair : data) {
    ...
    auto key_head = storage::BTreeNodeWithTimeStamp::GetHead(key.data(), sizeof(int));
    leng_t lower  = 0;
...
TEST_F(TestBTreeNodeWithTimeStamp, NormalOperationWithPrefix) {
...
TEST_F(TestBTreeNodeWithTimeStamp, TestCopyRange) {
  auto node  = new (&mem_) storage::BTreeNodeWithTimeStamp(true);
  auto other = std::make_unique<storage::BTreeNodeWithTimeStamp>(true);
...
TEST_F(TestBTreeNodeWithTimeStamp, TestSplit) {
  auto node     = new (&mem_) storage::BTreeNodeWithTimeStamp(true);
  auto new_leaf = std::make_unique<storage::BTreeNodeWithTimeStamp>(true);
  auto parent   = std::make_unique<storage::BTreeNodeWithTimeStamp>(false);
  std::vector<std::pair<int, int>> data;
  std::unordered_map<storage::BTreeNodeWithTimeStamp *, pageid_t> pid_map;
...
TEST_F(TestBTreeNodeWithTimeStamp, TestSplitInnerNodes) {
  auto left   = new (&mem_) storage::BTreeNodeWithTimeStamp(false);
  auto right  = std::make_unique<storage::BTreeNodeWithTimeStamp>(false);
  auto parent = std::make_unique<storage::BTreeNodeWithTimeStamp>(false);
...
TEST_F(TestBTreeNodeWithTimeStamp, TestMergeLeafNodes) {
  auto left   = new (&mem_) storage::BTreeNodeWithTimeStamp(true);
  auto right  = std::make_unique<storage::BTreeNodeWithTimeStamp>(true);
  auto parent = std::make_unique<storage::BTreeNodeWithTimeStamp>(false);
  std::vector<std::pair<int, int>> data;
  std::unordered_map<storage::BTreeNodeWithTimeStamp *, pageid_t> pid_map;
...
TEST_F(TestBTreeNodeWithTimeStamp, TestMergeInnerNodes) {
  auto left   = new (&mem_) storage::BTreeNodeWithTimeStamp(false);
  auto right  = std::make_unique<storage::BTreeNodeWithTimeStamp>(false);
  auto parent = std::make_unique<storage::BTreeNodeWithTimeStamp>(false);
  std::vector<std::pair<int, pageid_t>> data;
